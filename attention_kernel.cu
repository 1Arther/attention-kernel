...
void launch_flash_attention_skip_bm4_regacc_vec4_pcache_padding(
    ...
    if (D > MAX_D) {
        fprintf(stderr,
                "flash_attention_skip_bm4_regacc_vec4_pcache_padding only supports D <= %d, got D = %d\n",
                MAX_D,
                D);
        return;
    }
    ...
    flash_attention_causal_tile_skipping_regacc_vec4_pcache_padding_kernel<BLOCK_M, BLOCK_N, MAX_D>
        <<<grid, block>>>(
            d_Q,
            d_K,
            d_V,
...
void launch_flash_attention_skip_bm8_regacc_vec4_pcache_padding(
    ...
    if (D > MAX_D) {
        fprintf(stderr,
                "flash_attention_skip_bm8_regacc_vec4_pcache_padding only supports D <= %d, got D = %d\n",
                MAX_D,
                D);
        return;
    }
...
void launch_flash_attention_skip_bm16_regacc_vec4_pcache_padding(
    ...
    if (D > MAX_D) {
        fprintf(stderr,
                "flash_attention_skip_bm16_regacc_vec4_pcache_padding only supports D <= %d, got D = %d\n",
                MAX_D,
                D);
        return;
    }
